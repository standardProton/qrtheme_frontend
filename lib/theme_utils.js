...
export async function getOrders(con, user_id, theme_id){
    let orders = [];
    const order_res = await mysqlQuery(con, "SELECT id, output, qr_url, order_timestamp FROM orders WHERE owner = ? AND theme_id = ? AND (order_status > 1 OR order_status IS null)", [user_id, theme_id]);
    if (order_res != null && order_res.error_msg == null){
        const images = order_res.results;
        for (let i = 0; i < images.length; i++){
            orders.push({order_id: images[i].id, timestamp: images[i].order_timestamp, qr_url: images[i].qr_url, images: images[i].output.split(" ").filter(name => name.length > 0)});
        }
    }
