...
export async function getOrders(con, user_id, theme_id){
    const orders = [];
    const order_res = await mysqlQuery(con, "SELECT id, output, qr_url FROM orders WHERE owner = ? AND theme_id = ? AND (order_status > 1 OR order_status IS null)", [user_id, theme_id]);
