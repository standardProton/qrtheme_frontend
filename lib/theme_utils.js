...
export async function getOrder(con, pid){
    const id = getId(pid);
    const order_res = await mysqlQuery(con, "SELECT id, output, theme_id, qr_url, order_timestamp FROM orders WHERE id = ? AND (order_status > 1 OR order_status IS null)", [id]);
    if (order_res != null && order_res.error_msg == null && order_res.results.length > 0){
        const image = order_res.results[0]
       return {order_id: image.id, timestamp: image.order_timestamp, theme_id: image.theme_id, qr_url: image.qr_url, images: image.output.split(" ").filter(name => name.length > 0)};
    }
    return null;
}